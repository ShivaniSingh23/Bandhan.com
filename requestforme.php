...
$login_uid=$row['id'];
include('connection.php');
$sql="select * from reg inner join request on reg.id=request.reqby where request.reqto='$login_uid' and status='pending'";
$res=mysql_query($sql);
...
<div class="col-sm-3">
	...
	<a href="acceptrequest.php?id=<?php echo $row['id'];?>" class="btn btn-success" >Accept</a>  <a href="blockrequest.php?id=<?php echo $row['id'];?>" class="btn btn-danger" >Block</a></p>
	</div>
</div>
<?php
}
